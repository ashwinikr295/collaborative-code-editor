...
export const AdminDashboard: React.FC = () => {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen w-full bg-slate-950 text-slate-100 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-8 shadow-2xl flex flex-col gap-6 my-auto">
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-400 flex items-center gap-1.5">
                <KeyRound className="w-3.5 h-3.5 text-indigo-400" /> Secret Key
              </label>
              <input
                type="password"
                ...
                className="w-full px-4 py-2.5 rounded-xl bg-slate-950/80 border border-slate-800 focus:border-indigo-500/60 text-slate-200 text-sm placeholder-slate-500 outline-none transition-all"
              />
              {authError && <span className="text-xs text-rose-400 px-1 mt-0.5">{authError}</span>}
            </div>

            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-sm flex items-center justify-center gap-2 transition-all shadow-lg shadow-indigo-600/25 hover:shadow-indigo-500/35 cursor-pointer mt-2"
            >
          ...
          <button
            onClick={() => navigate('/')}
            className="text-xs text-slate-500 hover:text-slate-300 transition-colors flex items-center justify-center gap-1 cursor-pointer"
          >
            ← Back to Home
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 text-slate-100 p-6 md:p-10 font-sans selection:bg-indigo-500 selection:text-white overflow-y-auto">
      <div className="max-w-6xl mx-auto flex flex-col gap-8">
        ...
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              disabled={loading}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-800 text-xs font-semibold text-slate-300 hover:text-white transition-all cursor-pointer"
            >
            ...
            <button 
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-rose-950/20 hover:bg-rose-950/40 border border-rose-900/30 text-xs font-semibold text-rose-300 hover:text-rose-200 transition-all cursor-pointer"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          {/* Card 1: Active Rooms */}
          <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-5 flex flex-col gap-2 shadow-lg">
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-5 flex flex-col gap-2 shadow-lg">
            ...
            <div className="text-sm font-bold text-white mt-1 truncate" title={stats?.executionMode || ''}>
        ...
        <section className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 shadow-xl flex flex-col gap-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-4">
            <div>
              <h2 className="text-lg font-bold text-white">Code Execution Security Audit Trail</h2>
                ...
                <tbody className="divide-y divide-slate-800/40 text-slate-300">
                  {logs.map((log) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-3 font-semibold uppercase text-slate-400">
                        {log.language}
                      </td>
                      ...
                      <td className="py-3 px-3 font-mono text-slate-400 max-w-xs truncate">
        ...
        {selectedLog && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col gap-4 shadow-2xl animate-in fade-in duration-200">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="font-bold text-white text-sm flex items-center gap-2">
                <Terminal className="w-4 h-4 text-indigo-400" /> Log Details — Job ID: {selectedLog.id}
              </h3>
              <button 
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-mono bg-slate-950 p-3 rounded-xl border border-slate-800/60">
              <div><span className="text-slate-500">Room:</span> <span className="text-slate-200">{selectedLog.roomId}</span></div>
              <div><span className="text-slate-500">Language:</span> <span className="text-slate-200">{selectedLog.language}</span></div>
              <div><span className="text-slate-500">Duration:</span> <span className="text-slate-200">{selectedLog.executionTimeMs}ms</span></div>
              <div><span className="text-slate-500">Status:</span> {selectedLog.status}</div>
            </div>
            <div className="flex flex-col gap-2">
              <span className="text-xs font-semibold text-slate-400">Output Log Stream:</span>
              <pre className="p-4 rounded-xl bg-slate-950 border border-slate-800 font-mono text-xs text-emerald-400 overflow-x-auto max-h-48">
